        ...
        <header>
            <nav className={"h-20 w-full flex flex-row justify-between items-center fixed top-0 left-0 z-10 px-8 font-Raleway font-light"}>
                <ul className={"w-1/2 flex flex-row  space-x-5  justify-start"}>
                    <li className={"text-lg font-bold pr-2 "}>
                        <NavLink className={"flex flex-row items-center uppercase justify-start text-gray-900  "} to="/">
                        ...
                        return (
                            <li>
                                <NavLink to={item.url} className={({isActive}) =>
                                    isActive ? activeClass : "hover:underline underline-offset-8 hover:font-bold"}
                                >{item.name}</NavLink>
                            </li>
                        )
                    })}
