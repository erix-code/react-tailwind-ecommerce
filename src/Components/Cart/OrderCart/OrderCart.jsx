...
export default function OrderCart({product , removeProduct}) {
    ...
    return(
        <div className={"grid grid-cols-12 gap-5 items-center pr-2"}>
            ...
            <div className={"col-span-2 md:col-span-2"}>
                <input className={"border-2 border-gray-500 border-solid rounded-lg w-8 md:w-14 text-center"} type={"number"} value={product.quantity} readOnly={true} />
            </div>
            <span className={"text-sm md:text-md col-span-2"}>${product.totalAmount }</span>
            <RiCloseLargeLine  onClick={() => {removeProduct(product)}} className={"col-span-1 cursor-pointer"}></RiCloseLargeLine>
        </div>
    )
